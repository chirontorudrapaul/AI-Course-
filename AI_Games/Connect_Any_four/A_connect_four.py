...
pygame.display.set_caption("CONNECT ANY FOUR & WIN")
...
def show_menu():
    ...
    title = myfont.render("CONNECT ANY FOUR", 1, WHITE)
...
def main():
    ...
    while not game_over:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION and turn == PLAYER and not game_over:#Mouse Hover (Player's Turn Only)
                #Draws a floating red disc above the column where your mouse is
                pygame.draw.rect(screen, BLACK, (0, 0, width, SQUARESIZE))# Clear top row
                posx = event.pos[0] # Get X position of mouse
        ...
        if game_over:
            ...
            yes_no = smallfont.render("press (Y)for Yes & press(N) for NO", 1, WHITE)
